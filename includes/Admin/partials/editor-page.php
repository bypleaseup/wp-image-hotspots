...
?>
		<div class="wrap" id="wphs-wrap">
			...
			<div style="margin:0 0 20px 0;margin-right:16px;padding-bottom:16px;border-bottom:1px solid rgba(16,24,40,.08)">
				<h2 style="font-size:16px;font-weight:700;color:#0b1220;margin:0 0 6px 0"><?php esc_html_e( 'Editor', 'wp-image-hotspots' ); ?></h2>
				<p style="font-size:12px;color:#667085;margin:0"><?php esc_html_e( 'Select an image from the Media Library and click on it to place hotspots.', 'wp-image-hotspots' ); ?></p>
				<p style="font-size:12px;color:#667085;margin:4px 0 12px"><?php esc_html_e( 'Each hotspot can contain rich text. Use the shortcode [wphs_image id="X"] to embed the image anywhere on your site.', 'wp-image-hotspots' ); ?></p>
				<div style="display:flex;align-items:center;gap:10px">
					<button type="button" class="wphs-btn-primary wphs-btn-sm" id="wphs-pick">+ <?php esc_html_e( 'Choose image', 'wp-image-hotspots' ); ?></button>
				...
				<div class="wphs-card">
						...
						<div id="wphs-canvas" data-attachment="<?php echo esc_attr( (string) $attachment_id ); ?>">
							...
							<?php else : ?>
								<div id="wphs-canvas-empty">
									...
									<h2><?php esc_html_e( 'Select an image to start', 'wp-image-hotspots' ); ?></h2>
									<p><?php esc_html_e( 'Use the "Choose image" button above.', 'wp-image-hotspots' ); ?></p>
								</div>
							<?php endif; ?>
						</div>
					</div>
					...
					if ( ! empty( $recent ) ) : ?>
						...
						<div class="wphs-quick-header">
							<span class="wphs-quick-title"><?php esc_html_e( 'Images with hotspots', 'wp-image-hotspots' ); ?></span>
							<span class="wphs-quick-sub"><?php esc_html_e( 'Click to open', 'wp-image-hotspots' ); ?></span>
						</div>
							...
							<div class="wphs-thumb">
								<a href="<?php echo esc_url( $edit_url ); ?>" class="wphs-thumb-link">
									...
									printf(
										...
										esc_html( _n( '%d hotspot', '%d hotspots', $hs_count, 'wp-image-hotspots' ) ),
										(int) $hs_count
									);
								?></small>
								</a>
				...
				<div id="wphs-right-panel" <?php echo $attachment_id ? '' : 'style="display:none"'; ?>>
					...
					<div class="wphs-card">
						...
						<div class="wphs-meta-row">
							<span>
								<strong><?php esc_html_e( 'Last save:', 'wp-image-hotspots' ); ?></strong>
								<span id="wphs-saved-at" style="margin-left:4px"><?php echo $saved_at ? esc_html( $saved_at ) : esc_html__( 'never', 'wp-image-hotspots' ); ?></span>
							</span>
							<span class="wphs-count-pill" id="wphs-count">0</span>
						</div>

						<div class="wphs-action-row">
							<button type="button" class="wphs-btn-ghost wphs-btn-danger" id="wphs-del"><?php esc_html_e( 'Delete', 'wp-image-hotspots' ); ?></button>
							<button type="button" class="wphs-btn-ghost" id="wphs-clear"><?php esc_html_e( 'Remove all', 'wp-image-hotspots' ); ?></button>
							<button type="button" class="wphs-btn-primary" id="wphs-save" style="flex:1"><?php esc_html_e( 'Save', 'wp-image-hotspots' ); ?></button>
						</div>
						...
						<div id="wphs-editor-dock" class="wphs-editor-dock-hidden">
							...
							?>
							<p class="wphs-muted" style="margin-top:6px"><?php esc_html_e( 'Select a hotspot to edit its content.', 'wp-image-hotspots' ); ?></p>
						</div>
					...
					<div class="wphs-img-settings-panel" id="wphs-img-settings-panel">
						...
						<div class="wphs-settings-panel-head">
							<div style="display:flex;align-items:center;gap:7px">
								<h3><?php esc_html_e( 'Hotspot & Tooltip style', 'wp-image-hotspots' ); ?></h3>
								<span class="wphs-badge wphs-badge-override" id="wphs-img-badge"><?php esc_html_e( 'this image', 'wp-image-hotspots' ); ?></span>
							</div>
						</div>

						<?php /* Live preview */ ?>
						<div style="display:flex;justify-content:flex-end;margin-bottom:6px">
							<button type="button" id="wphs-preview-toggle" class="wphs-btn-secondary wphs-btn-sm" style="font-size:10px;padding:4px 10px;height:auto"><?php esc_html_e( 'Preview: normal', 'wp-image-hotspots' ); ?></button>
						</div>
						<div class="wphs-unified-preview" id="wphs-unified-preview">
							...
							<div class="wphs-preview-bubble" id="wphs-preview-bubble" style="position:relative;z-index:2">
								...
								<div class="wphs-tt-desc"><?php esc_html_e( 'Description…', 'wp-image-hotspots' ); ?></div>
							</div>
						</div>
						...
						<div id="wphs-img-settings-fields">
							...
							<?php /* Hotspot style */ ?>
							<div class="wphs-settings-section-label"><?php esc_html_e( 'Hotspot', 'wp-image-hotspots' ); ?></div>
							<div class="wphs-switch-group" id="wphs-img-style-switch">
								...
								<button type="button" class="wphs-switch-opt is-active" data-for="wphs-img-style-default"><?php esc_html_e( 'Default dot', 'wp-image-hotspots' ); ?></button>
								<button type="button" class="wphs-switch-opt" data-for="wphs-img-style-image" id="wphs-img-style-image-btn"><?php esc_html_e( 'Custom icon', 'wp-image-hotspots' ); ?></button>
							</div>
							<div class="wphs-icon-row" id="wphs-icon-row">
								<div style="display:flex;flex-direction:column;gap:6px;width:100%">
									<div style="display:flex;align-items:center;gap:8px">
										...
										<span class="wphs-fl" style="flex:1"><?php esc_html_e( 'Normal', 'wp-image-hotspots' ); ?> <span style="color:#0b1220;font-size:10px;font-weight:800"><?php esc_html_e( '(required)', 'wp-image-hotspots' ); ?></span></span>
										<button type="button" class="wphs-btn-secondary wphs-btn-sm" id="wphs-img-pick-icon"><?php esc_html_e( 'Choose', 'wp-image-hotspots' ); ?></button>
										<button type="button" class="wphs-btn-ghost wphs-btn-sm" id="wphs-img-remove-icon"<?php if(!$img_icon_url) echo ' disabled'; ?>><?php esc_html_e( 'Remove', 'wp-image-hotspots' ); ?></button>
									</div>
									<div style="display:flex;align-items:center;gap:8px">
										<?php
										...
										<span class="wphs-fl" style="flex:1"><?php esc_html_e( 'Selected', 'wp-image-hotspots' ); ?></span>
										<button type="button" class="wphs-btn-secondary wphs-btn-sm" id="wphs-img-pick-sel-icon"><?php esc_html_e( 'Choose', 'wp-image-hotspots' ); ?></button>
										<button type="button" class="wphs-btn-ghost wphs-btn-sm" id="wphs-img-remove-sel-icon"<?php if(!$sel_icon_url) echo ' disabled'; ?>><?php esc_html_e( 'Remove', 'wp-image-hotspots' ); ?></button>
									</div>
								</div>
							...
							<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:12px">
								...
								<div class="wphs-f" style="display:flex;flex-direction:column;gap:6px">
									<div style="display:flex;align-items:flex-end;gap:4px">
										<div class="wphs-f" style="flex:1"><span class="wphs-fl"><?php esc_html_e( 'W', 'wp-image-hotspots' ); ?></span><input type="number" id="wphs-img-dot-width" value="<?php echo absint( $img_settings['dot_width'] ); ?>" min="4" max="256" class="wphs-size-input" /></div>
										...
										<div class="wphs-f" style="flex:1"><span class="wphs-fl"><?php esc_html_e( 'H', 'wp-image-hotspots' ); ?></span><input type="number" id="wphs-img-dot-height" value="<?php echo absint( $img_settings['dot_height'] ); ?>" min="4" max="256" class="wphs-size-input" /></div>
									</div>
									<div class="wphs-f"><span class="wphs-fl"><?php esc_html_e( 'Radius (px)', 'wp-image-hotspots' ); ?></span><input type="number" id="wphs-img-dot-radius" value="<?php echo absint( $img_settings['dot_radius'] ); ?>" min="0" max="999" class="wphs-size-input" /></div>
								</div>
								...
								<div class="wphs-f wphs-dot-color-fields" id="wphs-dot-color-fields" style="display:flex;flex-direction:column;gap:8px">
									<div class="wphs-f"><span class="wphs-fl"><?php esc_html_e( 'Fill', 'wp-image-hotspots' ); ?></span><div class="wphs-color-row"><div class="wphs-color-swatch"></div><input type="color" id="wphs-img-dot-color" value="#000000" class="wphs-color-input" /><input type="text" id="wphs-img-dot-color-hex" value="#000000" class="wphs-hex-input" maxlength="7" /></div></div>
									<div class="wphs-f"><span class="wphs-fl"><?php esc_html_e( 'Border', 'wp-image-hotspots' ); ?></span><div class="wphs-color-row"><div class="wphs-color-swatch"></div><input type="color" id="wphs-img-dot-border-color" value="#ffffff" class="wphs-color-input" /><input type="text" id="wphs-img-dot-border-color-hex" value="#ffffff" class="wphs-hex-input" maxlength="7" /></div></div>
									<div class="wphs-f"><span class="wphs-fl"><?php esc_html_e( 'Fill selected', 'wp-image-hotspots' ); ?></span><div class="wphs-color-row"><div class="wphs-color-swatch"></div><input type="color" id="wphs-img-dot-selected-fill" value="<?php echo esc_attr( $img_settings['dot_selected_fill'] ); ?>" class="wphs-color-input" /><input type="text" id="wphs-img-dot-selected-fill-hex" value="<?php echo esc_attr( $img_settings['dot_selected_fill'] ); ?>" class="wphs-hex-input" maxlength="7" /></div></div>
									<div class="wphs-f"><span class="wphs-fl"><?php esc_html_e( 'Border selected', 'wp-image-hotspots' ); ?></span><div class="wphs-color-row"><div class="wphs-color-swatch"></div><input type="color" id="wphs-img-dot-selected-border" value="<?php echo esc_attr( $img_settings['dot_selected_border'] ); ?>" class="wphs-color-input" /><input type="text" id="wphs-img-dot-selected-border-hex" value="<?php echo esc_attr( $img_settings['dot_selected_border'] ); ?>" class="wphs-hex-input" maxlength="7" /></div></div>
								</div>
							...
							<div class="wphs-settings-section-label"><?php esc_html_e( 'Tooltip', 'wp-image-hotspots' ); ?></div>
							<div class="wphs-style-grid">
								<div class="wphs-style-field">
									<span class="wphs-style-lbl"><?php esc_html_e( 'Border radius (px)', 'wp-image-hotspots' ); ?></span>
									<input type="number" id="wphs-tt-radius" value="<?php echo absint( $tt_radius ); ?>" min="0" max="32" class="wphs-size-input" />
								</div>
								<div style="display:flex;flex-direction:column;gap:8px">
									<div class="wphs-style-field">
										<span class="wphs-style-lbl"><?php esc_html_e( 'Background', 'wp-image-hotspots' ); ?></span>
									...
									<div class="wphs-style-field">
										<span class="wphs-style-lbl"><?php esc_html_e( 'Text', 'wp-image-hotspots' ); ?></span>
										<div class="wphs-color-row">
											<div class="wphs-color-swatch"></div>
							...
							<div style="display:flex;gap:8px;margin-top:16px;padding-top:14px;border-top:1px solid rgba(16,24,40,.08)">
								<button type="button" class="wphs-btn-ghost" id="wphs-reset-img-settings" style="flex:1"><?php esc_html_e( 'Reset', 'wp-image-hotspots' ); ?></button>
								<button type="button" class="wphs-btn-primary" id="wphs-save-img-settings" style="flex:1"><?php esc_html_e( 'Save', 'wp-image-hotspots' ); ?></button>
							</div>
						</div>
					</div>
