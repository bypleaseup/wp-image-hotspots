...
final class AjaxHotspots extends AjaxBase {
	...
	public function save_hotspots() : void {
		...
		if ( ! is_array( $decoded ) ) {
			wp_send_json_error( array( 'message' => __( 'Invalid JSON', 'pleaseup-hotspots' ) ), 400 );
		}
	...
	public function save_hotspots_post() : void {
		if ( ! is_admin() ) {
			wp_die( esc_html__( 'Forbidden', 'pleaseup-hotspots' ) );
		}

		$attachment_id = isset( $_POST['attachment_id'] ) ? absint( $_POST['attachment_id'] ) : 0;
		if ( $attachment_id <= 0 ) {
			wp_die( esc_html__( 'Missing attachment_id', 'pleaseup-hotspots' ) );
		}

		check_admin_referer( 'wphs_save_hotspots_post_' . $attachment_id );

		if ( ! Capabilities::can_edit_attachment( $attachment_id ) ) {
			wp_die( esc_html__( 'Unauthorized', 'pleaseup-hotspots' ) );
		}
